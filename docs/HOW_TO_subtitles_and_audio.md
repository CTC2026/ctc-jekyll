...
The last line inside every `clip-section` is the **clip note** — the caption shown directly beneath the video (the `<p class="clip-caption">…</p>` on line 477 above). Unlike the subtitles and audio description, it is **not generated by any script — you write it by hand, one per clip.**
...
- **Description** — a short sentence saying who is on screen and what happens; name the characters when you can (e.g. "Mulan tries on her father's armor"). Write it yourself from watching the clip, or adapt the wording if the module's source document already describes it.
- **`Source:`** — where the video came from. Copy the clip's **Source** line from the module's **source document** (the module `.docx` / its PDF in the source-materials folder records each clip's source). Phrase it "The film on …" or "The production on …" with the site name linked, and keep `target="_blank"`.
- **`Credit:`** — include **only** if the source document gives one (e.g. `Credit: Clipped by CTC team.`). Otherwise leave it out.
