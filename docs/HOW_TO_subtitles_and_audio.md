...
### The clip note (the caption under each clip)

The last line inside every `clip-section` is the **clip note** — the caption shown directly beneath the video (the `<p class="clip-caption">…</p>` on line 477 above). Unlike the subtitles and audio description, it is **not generated by any script** — you add it to the page by hand, one per clip.

**The source — a `_ClipNotes.docx` per module.** Each module has a standalone **clip-notes document** that the **content author** fills in, exactly like the figures document — one per module, saved with the module's other source materials (e.g. `Feiyimeng_1964_OperaFilm_ClipNotes.docx`), with one entry per clip:

```
Clip 1: The opening scene of The Crimson Palm.
Source: The Crimson Palm on YouTube — https://www.youtube.com/watch?v=Z2RdZfkeo6k&t=4163s
Credit: Clipped by CTC team.
```

When you build the page, copy each clip's entry into its `<p class="clip-caption">` (below) and turn the Source into a link. If a module has no clip-notes document (older pages), write the note by hand from the clip and the essay.
...
- **`Clip N:`** — the clip's number on the page, counting from 1 in the order the clips appear.
- **Description** — a short sentence saying who is on screen and what happens; name the characters when you can (e.g. "Mulan tries on her father's armor"). Comes from the clip's entry in the `_ClipNotes.docx`; if the module has no clip-notes document, write it yourself from watching the clip.
- **`Source:`** — where the video came from. Take the clip's **Source** line from the `_ClipNotes.docx`, and phrase it "The film on …" or "The production on …" with the site name linked and `target="_blank"`.
- **`Credit:`** — include **only** if the clip's entry gives one (e.g. `Credit: Clipped by CTC team.`). Otherwise leave it out.

A clip with nothing to cite can be just the number and a description: `<p class="clip-caption">Clip 1: Reflection.</p>`

**Where it goes:** the clip note is the **last element inside** that clip's `<div class="clip-section">`, right before the closing `</div>`, so it renders directly under the video and its subtitle buttons.

**When to write it:** as you embed each clip (this Part) — do it for every clip, at the same time you add the subtitle and audio-description tracks above.

> **Using Claude Code:**
> *"In plays/…/[page].md, under the Clip N video, add a clip note: `Clip N: [description]. Source: The film on [site] ([URL]). Credit: [credit].` Use the `<p class=\"clip-caption\">` format and put it as the last line inside that clip's `clip-section`."*
