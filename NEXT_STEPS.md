...
> **📣 Update for new authors — you complete the whole video workflow yourself, except the upload.** For every clip on your page you are responsible for four things: **sourcing** a better-quality version of the clip, **upscaling** it for clarity with Topaz, creating the **subtitles** (`.vtt`, Chinese + English), and creating the **audio description**. Do all of these and hand off the finished files. The one step that is **not** yours is the **Cloudflare R2 upload** (Section 2 below) — that is a separate, more technical step handled by whoever has Cloudflare access. Guides for your part: [clip sourcing & alignment](docs/HOW_TO_clip_source_and_alignment.md), [Topaz upscaling](docs/HOW_TO_topaz_upscaling.md), [subtitles & audio description](docs/HOW_TO_subtitles_and_audio.md).
...
This is how the in-progress work in Section 1 gets finished — the upscaled clips, audio-description MP3s, and any images are served from the **ctc-media** R2 bucket, not from git. Do it **before** the accessibility test, since that test checks the media actually plays and has captions. See [HOW_TO.md Section 6](docs/HOW_TO.md).
...
- [ ] Upload audio-description MP3s the same way (see `upload_ad_to_r2.sh` as a template, and the [subtitles & audio guide](docs/HOW_TO_subtitles_and_audio.md))
- [ ] Upload images with `upload_images_to_r2.sh` — [HOW_TO.md Section 7, Step 4](docs/HOW_TO.md)
...
- [ ] If the page has video: source and upscale the clip, create subtitles and audio description, then **upload all media to R2 (Section 2)** — the same steps as the in-progress work in Section 1
